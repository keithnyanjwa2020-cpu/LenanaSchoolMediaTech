...
function purchaseItem(userId, itemId) {
  ...
  console.log('Purchase debug:', { userId, availableUsers: data.users.map(u => ({ id: u._id, name: u.name, username: u.username })) });
...
function giftPoints(fromId, toId, amount) {
  const data = readDB();
  const toUser = data.users.find(u => u._id === toId);
