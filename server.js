...
app.post('/api/shop/purchase', async (req, res) => {
  try {
    ...
    // Look up user by ID to get their current username
    const user = db.findUserById(decoded.id);
    if (!user) {
      console.log('User not found by ID:', decoded.id);
      return res.status(404).json({ error: 'User not found. Please log in again.' });
    }

    console.log('Purchase request:', { username: user.username, itemId });
    const result = db.purchaseItem(user.username, itemId);
...
app.post('/api/shop/equip', async (req, res) => {
  try {
    ...
    // Look up user by ID to get current username
    const user = db.findUserById(decoded.id);
    if (!user) return res.status(404).json({ error: 'User not found. Please log in again.' });

    const result = db.equipItem(user.username, itemId, slot);
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/admin/gift', async (req, res) => {
  try {
    ...
    console.log('Gift request:', { username, amount });

    if (!username || !amount) {
      return res.status(400).json({ error: 'Username and amount required' });
    }

    const result = db.giftPoints(decoded.id, username.toLowerCase(), parseInt(amount));
    if (result.error) return res.status(400).json({ error: result.error });
    res.json(result);
  } catch (err) {
