...
app.post('/api/shop/purchase', async (req, res) => {
  try {
    ...
    console.log('Purchase request:', { decodedId: decoded.id, itemId });
...
app.post('/api/admin/gift', async (req, res) => {
  try {
    ...
    const data = db.readDB();
    const targetUser = data.users.find(u => u.username === username.toLowerCase());

    if (!targetUser) {
      console.log('Available users:', data.users.map(u => u.username));
      return res.status(404).json({ error: 'User not found. Available: ' + data.users.map(u => u.username).join(', ') });
    }
